...
Array.prototype.shuffle = function() {
  for(var j, x, i = this.length-1; i; j = utils.randomNumber(i), x = this[--i], this[i] = this[j], this[j] = x);
  return this;
};
...
Array.prototype.roll = function () {
  var rand = utils.randomNumber(this.length);
  var tem = [];
...
let UNCHANGED_GENS = 0;
let mutationTimes = 0;
let doPreciseMutate = true;
...
let values = new Array(POPULATION_SIZE);
let fitnessValues = new Array(POPULATION_SIZE);
let roulette = new Array(POPULATION_SIZE);
let points = undefined

function GAInitialize(npoints) {
  population = []
  points = npoints
  countDistances();
...
function getChild(fun, x, y) {
  ...
  var dx,dy;
  var c = px[utils.randomNumber(px.length)];
  solution.push(c);
  while(px.length > 1) {
...
function doMutate(seq) {
  ...
  do {
    m = utils.randomNumber(seq.length - 2);
    n = utils.randomNumber(seq.length);
  } while (m>=n)
...
function pushMutate(seq) {
  ...
  var m,n;
  do {
    m = utils.randomNumber(seq.length>>1);
    n = utils.randomNumber(seq.length);
  } while (m>=n)
...
function countDistances() {
  ...
  for(var i=0; i<length; i++) {
    ...
    for(var j=0; j<length; j++) {
      dis[i][j] = ~~utils.distance(points[i], points[j]); 
    }
  }
}
...
module.exports = {
  ...
  GANextGeneration: GANextGeneration,
